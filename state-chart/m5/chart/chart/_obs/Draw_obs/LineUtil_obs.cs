...
public class LineUtil_obs
{
    ...
    #region 直線の重複
    public static int epsilon_overlapped=10;
    public static bool IsOverlapped(Point a, Point b, Point c, Point d, bool bHorizontal_or_Vertical)
    {
        var diff_ab = DrawUtil_obs.Sub_Point(a,b);
        var diff_cd = DrawUtil_obs.Sub_Point(c,d);

        var len_ab  = DrawUtil_obs.Len_Point(a,b);
        var len_cd  = DrawUtil_obs.Len_Point(c,d);
